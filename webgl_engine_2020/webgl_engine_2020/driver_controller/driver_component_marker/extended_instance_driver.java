...
public class extended_instance_driver extends instance_driver
{
	...
	public extended_instance_driver(component my_comp,int my_driver_id,
			component_marker_container my_cmc,int my_camera_modifier_id)
	{
	...
	{
		return (cr.target.main_display_target_flag||cr.target.selection_target_flag)?false:true;
	}
	...
	public void create_component_parameter(engine_kernel ek,client_information ci)
	{
		...
		for(int i=0,ni=cmc.component_marker_array.length;i<ni;i++) {
			...
			component my_comp=ek.component_cont.search_component(p.marker_component_name);
			if(my_comp==null)
				continue;
	...
	public String[] response_event(int parameter_channel_id,engine_kernel ek,client_information ci)
	{
		...
		case "jason":
			...
			for(int i=0,ni=cmc.component_marker_array.length;i<ni;i++) {
				component_marker p=cmc.component_marker_array[i];
				operate_comp=ek.component_cont.search_component(p.marker_component_name);
				int component_id=(operate_comp==null)?-1:(operate_comp.component_id);
				ci.request_response.println("	{");
			...
			break;
		case "clear_all":
			cmc.clear_all_component_marker();
			update_component_parameter_version(0);
			break;
		case "clear":
			if((str=ci.request_response.get_parameter("marker_id"))==null)
				break;
			cmc.clear_component_marker(Long.parseLong(str));
			update_component_parameter_version(0);
			break;
		case "delete":
		case "swap_select":
		case "locate":
			...
			if((operate_comp=ek.component_cont.search_component(operate_cm.marker_component_name))==null)
				break;	
			switch(str){
			case "delete":
				cmc.delete_component_marker(ci.parameter.body_id);
				update_component_parameter_version(0);
				break;
			case "swap_select":
			...
			case "locate":
				new locate_camera(ek.camera_cont.camera_array[ci.display_camera_result.target.camera_id]).
						locate_on_components(ek.modifier_cont[camera_modifier_id],
		...
		case "add":
			...
			ci.request_response.println(cmc.append_component_marker(operate_comp,
