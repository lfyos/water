...
public class movement_function_switch
{
	...
	private long locate_camera()
	{
		...
		if(searcher.result_parent!=null){
			manager.movement_start(
				ek.modifier_cont[manager.config_parameter.movement_modifier_id],
			...
			if((all_components.get_box()!=null)&&(ci.display_camera_result.cam.parameter.movement_flag))
				(new locate_camera(ci.display_camera_result.cam)).locate_on_components(
						ek.modifier_cont[manager.config_parameter.movement_modifier_id],all_components.get_box(),
	...
	private void reload()
	{
		if(manager!=null){
			...
			manager.init(ek.modifier_cont[manager.config_parameter.movement_modifier_id],
	...
	private long component_part_selection()
	{
		...
		case "clear":
			class clear_component_part_selection
			{
				public void process(movement_tree t)
				{
					...
					if(t.match!=null)
						t.match.add_or_delete_component_part_selection(false);
				}
			}
			(new clear_component_part_selection()).process(searcher.result);
			break;
		case "set":
			if(searcher.result.children!=null)
			...
			if(searcher.result.match!=null)
				searcher.result.match.add_or_delete_component_part_selection(true);
			break;
		}
		return searcher.result.movement_tree_id;
	}
	...
	private String[] design_request_dispatch(int movement_component_id,int movement_driver_id)
	{
		...
		case "save":
			...
			manager.flush(ek.component_cont,
					ek.modifier_cont[manager.config_parameter.movement_modifier_id],f,switch_time_length);
			f.close();
			return null;
		case "reload":
			push();
			...
			return null;
		case "retreat":
			manager.pop_movement(ek.modifier_cont[manager.config_parameter.movement_modifier_id],
	...
	private String[]  movement_request_dispatch()
	{
		...
		switch(str){
		case "stop":
			ek.modifier_cont[manager.config_parameter.movement_modifier_id].clear_modifier(ek,ci);
		...
		case "continue":
			...
			manager.create_render_modifier(single_step_flag,
					manager.config_parameter.movement_modifier_id,
