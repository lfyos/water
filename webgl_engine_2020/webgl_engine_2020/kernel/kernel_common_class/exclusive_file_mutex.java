package kernel_common_class;

import java.io.File;
import java.io.RandomAccessFile;
import java.nio.channels.FileChannel;
import java.nio.channels.FileLock;
import java.nio.channels.OverlappingFileLockException;

import kernel_file_manager.file_writer;

public class exclusive_file_mutex
{
	private String file_name;
	private RandomAccessFile f_out;
	private FileChannel f_channel;
	private FileLock f_lock;
	private long sleep_time_length,max_time_length;
	
	private exclusive_file_mutex(String my_file_name)
	{
		file_name=my_file_name;
		f_out=null;
		f_channel=null;
		f_lock=null;
		
		sleep_time_length=4;
		max_time_length=64*1024;
		
		if(new File(file_name).exists())
			return;
		file_writer.make_directory(file_name);
		try{
			(new File(file_name)).createNewFile();
		}catch(Exception e) {
			;
		}
	}
	private void wait_for_a_while()
	{
		sleep_time_length=((long)Math.round(2*sleep_time_length))%max_time_length;
		long this_sleep_time_length=(long)(Math.random()*sleep_time_length);
		try {
			Thread.sleep(this_sleep_time_length);
			return;
		}catch(Exception e) {
			debug_information.println(
				"lock exclusive_file_mutex:sleep_time_length=",
				this_sleep_time_length+"\t"+e.toString());
			e.printStackTrace();
		}
		Thread.yield();
	}
	private exclusive_file_mutex lock_routine(boolean try_flag)
	{
		for(File f=new File(file_name);;wait_for_a_while()){
			try{
				if(f_out==null)
					f_out=new RandomAccessFile(f,"rw");
		    	if(f_channel==null)
		    		f_channel=f_out.getChannel();
		    	
		    	if(f_lock==null) {
		    		if(try_flag)
		    			f_lock=f_channel.tryLock();
		    		else
		    			f_lock=f_channel.lock();
		    	}
		    	
		    	if(f_lock!=null)
					if(f_lock.isValid())
						return this;
		    	
		    }catch(OverlappingFileLockException e){
		    	;
			}catch(Exception e) {
		    	debug_information.println(
		    			"Create file lock fail in exclusive_file_mutex: ",
		    			file_name+"\t\t"+e.toString());
		    	e.printStackTrace();
		    }
			unlock();
			if(try_flag)
				return null;
		}
	}
	public void unlock()
	{
		if(f_lock!=null){
			try{
	    		f_lock.release();
			}catch(Exception e){
				debug_information.println("f_lock.release() fail in exclusive_file_mutex: ",
						file_name+"\t"+e.toString());
				e.printStackTrace();
			}
			try{
	    		f_lock.close();
			}catch(Exception e){
				debug_information.println("f_lock.close() fail in exclusive_file_mutex: ",
						file_name+"\t"+e.toString());
				e.printStackTrace();
			}
			f_lock=null;
		}
		if(f_channel!=null){
			try{
				f_channel.close();
			}catch(Exception e){
				debug_information.println("f_channel.close() fail in exclusive_file_mutex: ",
						file_name+"\t"+e.toString());
				e.printStackTrace();
			}
			f_channel=null;
		}
		if(f_out!=null){
			try{
				f_out.close(); 
			}catch(Exception e){
				debug_information.println("f_out.close() fail in exclusive_file_mutex: ",
						file_name+"\t"+e.toString());
				e.printStackTrace();
			}
			f_out=null;
		}
	}
	public static exclusive_file_mutex lock(String my_file_name)
	{
		return (new exclusive_file_mutex(my_file_name)).lock_routine(false);
	}
	public static exclusive_file_mutex try_lock(String my_file_name)
	{
		return (new exclusive_file_mutex(my_file_name)).lock_routine(true);
	}
}
