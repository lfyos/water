...
public class movement_tree {
	...
	public int movement_tree_id;
	public movement_tree search_movement(int my_movement_tree_id)
	{
		movement_tree t;
	...
	public void register_modifier(movement_channel_id move_channel_id,int location_component_id,
			...
			int  parent_scale_type,double parent_scale_value,boolean direction_flag)
	{
		...
		if(children!=null){
			for(int i=0;i<(children.length);i++)
				children[i].register_modifier(move_channel_id,location_component_id,
		...
		if(move==null)
			return;
		component moved_component=component_cont.get_component(move.moved_component_id);
		if(moved_component==null)
			return;
		if(move.movement.length<=0)
			return;

		move.register_modifier(move_channel_id,location_component_id,component_cont,modifier_cont);
	...
	public int set_tree_node_id(int my_id)
	{
		if(children!=null)
			for(int i=0;i<(children.length);i++)
				my_id=children[i].set_tree_node_id(my_id);
		movement_tree_id=my_id++;
		return my_id;
	}
	...
	public void flush(file_writer f,int space_number,boolean flush_match_flag,int flush_number)
	{
		if(node_name==null)
			node_name="no_name";
		...
		if(children==null){
			...
			for(int i=0;i<(children.length);i++)
				 children[i].flush(f,space_number+8,flush_match_flag,flush_number);
		}
	}
	public movement_tree(file_reader f)
	{
		String str;
		move=null;
		children=null;
		...
		if(child_number<=0){
			...
			for(String my_follow_component_name;;){
				...
				if(follow_component_name==null){
					...
					follow_component_location	=new location[1];
					follow_component_location[0]=new location(f);
				}else{
			...
			match=new movement_match_container(f,true);
			sequence_flag=true;
		}else{
			for(i=0,children=new movement_tree[child_number];i<child_number;i++)
				children[i]=new movement_tree(f);
		}
	}
}
