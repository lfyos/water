...
public class movement_load_old_file {
	...
	public movement_tree combine_to_tree()
	{
		int i,j,k,m;
		
		movement_tree root_node=new movement_tree();
		...
		for(i=0;i<max_movement;i=j){
			movement_tree main_node=new movement_tree();
			...
			for(j=i;j<max_movement;j=k){
				if(main_step_array[i]!=main_step_array[j])
					break;
				movement_tree min_node=new movement_tree();
	...
	private movement_tree load_one_node(file_reader f)
	{
		String my_system_name;
		
		movement_tree ret_val=new movement_tree();
		...
		ret_val.match			=new movement_match_container(f,false);
	...
	public movement_load_old_file(file_reader f)
	{
		...
		for(max_movement=0;max_movement<(move_array.length);max_movement++){
			move_array[max_movement]=load_one_node(f);
