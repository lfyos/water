...
public class movement_searcher
{
	...
	private boolean search(movement_tree my_current,movement_tree my_current_parent,int movement_tree_id,int my_result_id)
	{
		if(my_current!=null){
			...
			if(my_current.children!=null)
				for(int i=0;i<(my_current.children.length);i++)
					if(search(my_current.children[i],my_current,movement_tree_id,i))
						return true;
		}
		return false;
	}
	public movement_searcher(movement_tree root_movement,int movement_tree_id)
	{
		result=null;
		result_parent=null;
