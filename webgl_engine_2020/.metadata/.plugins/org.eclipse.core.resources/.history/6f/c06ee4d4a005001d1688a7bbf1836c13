...
public class engine_interface
{
				...
				if(engine_kernel_cont.ek!=null) {
					...
					if(engine_kernel_cont.ek.component_cont!=null)
						if(engine_kernel_cont.ek.component_cont.root_component!=null)
							statistics_interface.engine_component_number-=
								engine_kernel_cont.ek.component_cont.root_component.component_id+1;
					engine_kernel_cont.ek.destroy();
					engine_kernel_cont.ek=null;
				}
				engine_kernel_cont=null;
			};
			debug_information.println("engine_interface	engine_kernel_number:	",		statistics_interface.engine_kernel_number);
			debug_information.println("engine_interface	engine_component_number:	",	statistics_interface.engine_component_number);
		}
			...
			interface_statistics statistics_interface,system_parameter system_par)
	{
		...
		debug_information.print  ("engine_interface engine_kernel_number:	",		statistics_interface.engine_kernel_number);
		debug_information.println("/",system_par.max_engine_kernel_number);
		debug_information.print  ("engine_interface engine_component_number:	",	statistics_interface.engine_component_number);
		...
		if((ekbti.engine_kernel_cont=new engine_kernel_container(scene_name,link_name,
			...
			original_render,part_loader_cont,statistics_interface)).ek==null)
		{
	...
	public void destroy_scene(String my_scene_name,String my_link_name,interface_statistics statistics_interface)
	{
		client_interface_lock.lock();
		while(bt!=null) {
			...
			ekbtl.destroy(statistics_interface);
			break;
		}
		client_interface_lock.unlock();
	}
	private void destroy_engine_kernel_balance_tree(balance_tree ek_bt,interface_statistics statistics_interface)
	{
		if(ek_bt!=null){
			destroy_engine_kernel_balance_tree(ek_bt.get_left_child(),statistics_interface);
			destroy_engine_kernel_balance_tree(ek_bt.get_right_child(),statistics_interface);
			balance_tree_item bti=ek_bt.get_item();
			if(bti!=null)
				if(bti instanceof engine_kernel_balance_tree_item)
					((engine_kernel_balance_tree_item)bti).destroy(statistics_interface);
		}
	}
	public void destroy(interface_statistics statistics_interface)
	{
		...
		if(bt!=null) {
			destroy_engine_kernel_balance_tree(bt,statistics_interface);
			bt.destroy();
			bt=null;
		}
