	...
	public boolean touch_distance_tag(engine_kernel ek,client_information ci)
	{
		...
		for(int i=0,ni=distance_tag_array.length;i<ni;i++) {
			...
			case 0:	//input second point
				...
				my_point=ci.display_camera_result.matrix.multiply(p.px);
				...
				p.py=p.py.expand(my_point.distance()/100).add(p.p0);
			...
			case 1://confirm third point
				...
				if((p0_pl=new plane(p.p0,p.px)).error_flag)
					return true;
				if((p_up_pl=new plane(p.p0,p.px,p.px.add(ci.selection_camera_result.up_direct))).error_flag)
					return true;
				my_point=p_up_pl.insection_point(
