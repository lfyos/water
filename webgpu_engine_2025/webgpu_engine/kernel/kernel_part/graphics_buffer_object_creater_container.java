...
public class graphics_buffer_object_creater_container
{
			...
			long my_create_buffer_object_bitmap)
	{
		...
		if(p_creater==null) {
			...
			p_creater=new graphics_buffer_object_creater(
					my_file_name+".txt",my_file_charset,
					my_create_buffer_object_bitmap);
	...
	{
		ArrayList<graphics_buffer_object_creater> my_list;
		graphics_buffer_object_creater my_creater;
		
		for(int i=0,ni=creaters.size();i<ni;i++) 
			if((my_creater=(my_list=creaters.get(i)).get(my_list.size()-1))!=null)
				my_creater.test_end(0,true);
		
		head_fw.println("\t\t\t\"region_data\"\t:\t[");
		
		int file_number=0;
		for(int i=0,ni=creaters.size(),file_id=0;i<ni;i++)
			for(int j=0,nj=(my_list=creaters.get(i)).size();j<nj;j++) {
				if((my_creater=my_list.get(j))==null)
					continue;
				long item_number=my_creater.get_item_number();
				if(item_number<=0)
					continue;
				if(file_id>0)
					head_fw.println(",");
				String creater_file_name=my_creater.create_head_data(head_fw,i);
