...
public class caculate_material_id
{
	public static int caculate(part_driver pd,int max_material_id,
			part p,String type_str,int body_id,int face_id,int loop_id,int edge_id,String material[])
	{
		int material_id=0;
		try {
			material_id=pd.caculate_material_id(p,type_str,body_id,face_id,loop_id,edge_id,material);
		}catch(Exception e) {
			e.printStackTrace();
