...
public class class_file_reader
{
	...
	public static long get_last_time(String file_name,Class<?>first_class,String jar_file_charset)
	{
		File f;
		...
		for(Class<?> my_class=first_class;my_class!=null;my_class=my_class.getSuperclass()){
			...
			if((class_directory_name=uri.getPath())==null)
				continue;
			if((f=new File(file_reader.separator(class_directory_name+file_name))).exists())
				return f.lastModified();
		}
		for(Class<?> my_class=first_class;my_class!=null;my_class=my_class.getSuperclass()){
			String path=my_class.getProtectionDomain().getCodeSource().getLocation().getPath();
			try{
				path=file_reader.separator(java.net.URLDecoder.decode(path,jar_file_charset));
			}catch(Exception e){
				continue;
			}
			if((f=new File(file_reader.separator(path))).exists())
				return f.lastModified();
		}
		return 0;
	}
