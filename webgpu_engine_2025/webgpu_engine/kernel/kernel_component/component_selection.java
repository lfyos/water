...
public class component_selection
{
	...
	private void clear_selected_flag_without_brother(component my_comp)
	{
		...
		for(;my_comp!=null;my_comp=component_cont.get_component(my_comp.parent_component_id)){
			component_link_list_head=new component_link_list(my_comp,0,component_link_list_head);
			if(my_comp.uniparameter.selected_flag){
				...
				for(;component_link_list_head.next_list_item!=null;
					...
					for(int i=0,ni=component_link_list_head.comp.children.size();i<ni;i++) {
						my_comp=component_link_list_head.comp.children.get(i);
						if(component_link_list_head.next_list_item.comp!=my_comp)
							my_comp.uniparameter.selected_flag=true;
					}
				return;
			}
