...
public class component_selection
{
	...
	{
		for(component_link_list p=null;my_comp!=null;
				my_comp=component_cont.get_component(my_comp.parent_component_id))
			if((p=new component_link_list(my_comp,0,p)).comp.uniparameter.selected_flag){
				do_clear_selected_flag(p.comp);
				for(component my_child;p.next_list_item!=null;p=p.next_list_item)
					for(int i=0,ni=p.comp.children.size();i<ni;i++)
						if(p.next_list_item.comp!=(my_child=p.comp.children.get(i)))
							do_set_selected_flag(my_child);
				return;
			}
	}
	...
	public void clear_selected_flag()
	{
	...
	public void set_parent_selected(component my_comp)
	{
		...
		set_selected_flag((my_parent==null)?my_comp:my_parent);
	}
	...
	public void set_moved_descendant_selected(component comp)
	{
		if(comp!=null){
			if(comp.move_location.is_not_identity_matrix())
				set_selected_flag(comp);
			else
				for(int i=0,n=comp.children.size();i<n;i++)
					set_moved_descendant_selected(comp.children.get(i));
		}
	}
