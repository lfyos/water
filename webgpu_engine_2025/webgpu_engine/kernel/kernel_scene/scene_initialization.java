...
public class scene_initialization
{
	...
	private long initialization_program_last_time(
			ArrayList<component> init_comp,scene_kernel sk,client_process_bar process_bar)
	{
		long last_time=0;
	...
	private long initialization_last_time(ArrayList<component> init_comp,
			scene_kernel sk,client_process_bar process_bar)
	{
		...
		if(last_time<(my_last_time=shader_program_last_time(sk)))
			last_time=my_last_time;
		if(last_time<(my_last_time=initialization_program_last_time(init_comp,sk,process_bar)))
			last_time=my_last_time;
		return last_time;
	}
			...
			for(int j=0,nj=comp.initialization.program_and_charset.size();j<nj;j++) {
				...
				if(my_program_and_charset[1]==null){
					...
					new output_all_component_program(
							comp,ocp,my_program_and_charset[0],
