...
public class scene_initialization
{
	...
	private void  render_driver_initialize(scene_kernel sk,
	...
	private void part_driver_initialize(scene_kernel sk,
			client_request_response request_response,client_process_bar process_bar)
	{
		...
		for(int i=0;i<number;i++){
			...
			if(my_p.driver!=null)
				try {
					my_p.driver.initialize_part_driver(my_p,sk,request_response);
				}catch(Exception e){
					e.printStackTrace();
	...
	private void initialize_component_driver(component sort_component_array[],scene_kernel sk,
			client_request_response request_response,client_process_bar process_bar)
	{
	...
	private ArrayList<component> file_initialize(String destination_file_name,component sort_component_array[],
			scene_kernel sk,client_request_response request_response,client_process_bar process_bar)
	{
		long last_time,my_last_time=program_file_reader.get_system_program_last_time(sk.system_par);
		
		if((last_time=sk.caculate_scene_last_modified_time())<my_last_time)
