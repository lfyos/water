...
public class extended_instance_driver extends instance_driver
{
			...
			component comp,int control_code,engine_kernel ek,client_information ci)
	{
		...
		component_collector collector=(new list_component_on_collector(
			false,true,false,false,((function_id%2)==0)?true:false,false,ek,ci,cam_result)).collector;
	...
	private void select_single_component(component comp,int control_code,engine_kernel ek,client_information ci)
	{
		...
		case 3:
			(new locate_camera(ci.display_camera_result.cam)).locate_on_components(
					ek.modifier_cont[modifier_container_id],ek.component_cont,ci.display_camera_result,ci.parameter,null,
					ci.display_camera_result.cam.parameter.scale_value,ci.parameter.aspect,
					ek.modifier_cont[modifier_container_id].get_timer().get_current_time(),true,true,true,null,null);
			break;
		}
		if(control_code==0)
			return;
