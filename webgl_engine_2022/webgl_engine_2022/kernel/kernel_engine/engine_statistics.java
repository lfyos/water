package kernel_engine;

public class engine_statistics 
{
	public  int 	responsing_file_data_number;
	public  int 	responsing_network_data_number;
	
	public  long	file_download_number;
	public  long	file_download_data_uncompress_length;
	public  long	file_download_data_compress_length;
	
	public  long	network_data_number;
	public  long	network_data_uncompress_length;
	public  long	network_data_compress_length;
	
	public  int		proxy_request_file_number;
	public  long	proxy_request_data_length;
	public  int		proxy_response_file_number;
	public  long	proxy_response_data_length;
	
	public  int		engine_kernel_number;
	public  int		engine_component_number;
	
	synchronized public void update_kernel_component_number(int kernel_number,int component_number)
	{
		engine_kernel_number+=kernel_number;
		engine_component_number+=component_number;
	}
	public void clear()
	{
		responsing_file_data_number=0;
		responsing_network_data_number=0;
		
		file_download_number=0;
		file_download_data_uncompress_length=0;
		file_download_data_compress_length=0;
		
		network_data_number=0;
		network_data_uncompress_length=0;
		network_data_compress_length=0;
		
		proxy_request_file_number=0;
		proxy_request_data_length=0;
		
		proxy_response_file_number=0;
		proxy_response_data_length=0;
	}
	public engine_statistics()
	{
		clear();
		engine_kernel_number=0;
		engine_component_number=0;
	}
}
