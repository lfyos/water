...
public class extended_part_driver extends part_driver
{
			...
			system_parameter system_par,scene_parameter scene_par)
	{
		if(buffer_object_file_writer!=null) {
			String file_name=p.directory_name+file_reader.separator(p.material_file_name);
			file_reader.get_text(buffer_object_file_writer,file_name,p.file_charset);
		}
		return super.create_part_mesh_and_buffer_object_head(
				p,buffer_object_file_writer,pcps,system_par,scene_par);
	}
