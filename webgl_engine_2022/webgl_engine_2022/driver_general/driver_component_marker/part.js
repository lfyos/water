function construct_part_driver(init_data,part_object,render_driver,render)
{
	this.decode_vertex_data=function(request_type_string,buffer_object_data,part_object)
	{
		var ret_val=
		{
			...
			region_data		:	(request_type_string=="frame")?new Array():buffer_object_data.region_data,
			item_number		:	(request_type_string=="frame")?0:(buffer_object_data.item_number),
			item_size		:	(request_type_string=="frame")?4:(buffer_object_data.item_size),
			private_data	:	null
		};
		return ret_val;
	}
