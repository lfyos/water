		...
		client_information ci,int requesting_number,int max_request_number)
	{
		...
		for(int i=requesting_number;i<max_request_number;i++){
			...
			case 1:
				...
				break;
			default:
				package_file_name		=ek.render_cont.type_part_package[part_type_id-2].package_file_name[part_package_id];
				package_render_part_id	=ek.process_part_sequence.type_package_render_part_id.get(part_type_id-2).get(part_package_id);
				package_length			=ek.render_cont.type_part_package[part_type_id-2].package_length[part_package_id];
				break;
			}
