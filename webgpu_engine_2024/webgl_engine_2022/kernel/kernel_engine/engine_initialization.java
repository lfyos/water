...
public class engine_initialization
{
	...
	{
		long my_last_time,last_time=program_file_reader.get_system_program_last_time(ek.system_par);
				...
				for(int j=0,nj=shader_file_name[i].length;j<nj;j++){
					...
					if(shader_file_name[i][j].substring(index_id,index_id+3).toLowerCase().compareTo(".js")==0)
						my_last_time=class_file_reader.get_last_time(shader_file_name[i][j],
									r.driver.getClass(),ek.system_par.js_jar_file_charset);
					else if(shader_file_name[i][j].substring(index_id,index_id+4).toLowerCase().compareTo(".txt")==0)
						my_last_time=class_file_reader.get_last_time(shader_file_name[i][j],
									r.driver.getClass(),ek.system_par.text_jar_file_charset);
					else
						my_last_time=class_file_reader.get_last_time(shader_file_name[i][j],
									r.driver.getClass(),ek.system_par.local_data_charset);
