...
{
	private static component_collector get_collector(scene_kernel sk,client_information ci)
	{
		...
		if((str=ci.request_response.get_parameter("list_id"))!=null)
			return sk.collector_stack.get_collector_by_list_id(Long.decode(str));
		if((str=ci.request_response.get_parameter("list_title"))!=null){
			...
			return sk.collector_stack.get_collector_by_list_title(str);
		}
		return sk.collector_stack.get_top_collector();
	}
	private static void part_list_selection(int modifier_container_id,int select_type_id,
			String request_charset,scene_kernel sk,client_information ci,
			driver_audio_player.extended_component_driver acd)
	{
		...
		if((str=ci.request_response.get_parameter("component_id"))!=null) {
			my_comp=sk.component_cont.get_component(Integer.decode(str));
			...
			my_comp=sk.component_cont.search_component(str);
		}
		if(my_comp!=null){
			...
			if(comp_con.comp_list.size()>0)
				if(select_type_id!=2)
					(new locate_camera(ci.display_camera_result.cam)).locate_on_components(
						sk.modifier_cont[modifier_container_id],comp_con.get_box(),null,
	...
	public static void part_list_request(
			...
			scene_kernel sk,client_information ci,driver_audio_player.extended_component_driver acd)
	{
		String str,request_charset=ci.request_response.implementor.get_request_charset();
		...
		case "part_list_jason_component":
			...
			new component_collector_jason_component(
				str.compareTo("true")==0,sk.collector_stack.get_top_collector(),ci,sk);
			return;
		case "selection":
			...
			switch(str.toLowerCase()){
			case "select":
				part_list_selection(modifier_container_id,1,request_charset,sk,ci,acd);
				return;
			case "unselect":
				part_list_selection(modifier_container_id,2,request_charset,sk,ci,acd);
				return;
			case "swap":
				part_list_selection(modifier_container_id,3,request_charset,sk,ci,acd);
				return;
			default:
				return;
			}
		case "title":
		{
			...
			if(flag>0)
				sk.collector_stack.update_collector_version();
			return;
		}
		case "delete":
			if((collector=get_collector(sk,ci))==null)
		...
		case "save":
			file_writer fw=new file_writer(
					sk.collector_stack.component_collector_stack_file_name,
					sk.collector_stack.component_collector_stack_file_charset);
			sk.collector_stack.save(fw,sk.component_cont,
		...
		case "download":
			sk.collector_stack.save(ci.request_response,sk.component_cont,
				ci.request_response.get_boolean("top",false),save_component_name_or_id_flag);
			return;
		case "upload":
		{
